...
def load_model():
    ...
    if not model_path.exists():
        st.error(
            "Modèle introuvable. Exporte d'abord le modele dans le notebook:"
            "`artifacts/flight_price.joblib`."
        )
        st.stop()
...
TIME_BANDS = ['Night', 'Evening', 'Morning', 'Afternoon', 'Early_Morning','Late_Night']  
STOPS_LABEL_TO_NUM = {"zero":0, "one":1, "two_or_more":2}
CLASS_LABEL_TO_NUM = {"Economy":0, "Business":1}

def make_row(airline, source_city, destination_city,departure_time, arrival_time, duration, days_left, stops_label,class_label):
    return pd.DataFrame([{
        "airline": airline,
